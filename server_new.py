...
#logging
logger = lg.basicConfig(filename = "server_logs_"+str(datetime.now().date())+".log", 
                        level = lg.INFO, 
...
casting_model_path = 'yolov5/2022_04_08_cam1_big_dross.pt'
model = torch.hub.load("yolov5","custom",path = casting_model_path,source = 'local',force_reload=True,)  # or yolov5m, yolov5l, yolov5x, custom
# Yolov5 Stacking Model Load
stacking_model_path = 'yolov5-master_model2/yolov5-master/2022_03_26_cam2_retrained_last.pt'
model2 = torch.hub.load("yolov5-master_model2/yolov5-master",
                        "custom",path = stacking_model_path,source = 'local',force_reload=True,)  # or yolov5m, yolov5l, yolov5x, custom
#empty tray model
loaded_model = tf.keras.models.load_model('model_empty_classifier.h5')

#legged model
legged_model = tf.keras.models.load_model('hzl_leg_ingot_SVM.h5')
...
if __name__ == "__main__":
    ...
    app.run(host='localhost', port=args.port,debug=True)  # debug=True causes Restarting with stat
